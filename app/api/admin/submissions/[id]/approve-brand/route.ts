...
export async function POST(
  ...
  try {
    ...
    const nonWoodCells = await prisma.nonWoodPricingCell.findMany({
      where: { brandId }
    })

    // 2. Create the quote copy for this brand with APPROVED status
    ...
    const itemsToCreate = submission.items.map((item) => {
      ...
      if (item.itemType && item.hardware && item.coreMaterial && item.externalFinish) {
        ...
        const cell = nonWoodCells.find(c => c.itemType === item.itemType && c.unit === unit)
        if (cell) {
    ...
    await createAuditLog({
      action: 'SUBMISSION_DIRECT_APPROVED',
      entityType: 'quote',
      entityId: id,
      performedBy: admin.name || 'Admin',
      details: `Admin directly approved matrix-calculated quote for brand ${brandId}`,
    })

    return NextResponse.json({ success: true, quoteId: brandQuote.id })
  } catch (error) {
    console.error('Admin direct approve error:', error)
